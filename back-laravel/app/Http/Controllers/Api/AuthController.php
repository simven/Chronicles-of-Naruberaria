...
class AuthController extends Controller {
    ...
    public function register(Request $request) {
        ...
        try {
            ...
            $this->success['player'] = new PlayerResource($user->player);
            $this->success['token'] = $user->createToken('Game-api', [$user->role()->first()->role])->accessToken;
        });
        } catch (Exception $e) {
    ...
    public function login() {
        if (Auth::attempt(['email' => request('email'), 'password' => request('password')])) {
            ...
            $success['player'] = new PlayerResource($user->player);
            $success['token'] = $user->createToken('Game-api', [$this->scope])->accessToken;
